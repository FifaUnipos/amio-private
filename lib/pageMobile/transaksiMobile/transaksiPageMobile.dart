...
class _TransaksiMobilePageState extends State<TransaksiMobilePage>
  ...
  final String _getVariantLink = ApiEndpoints.getVariantLink;
  ...
  Future<void> _fetchProducts() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _products = result.map((e) => _mapModelToProduct(e)).toList();
          _filteredProducts = _products;
          _isLoading = false;
        });
      }
  ...
  Future<List<ProductVariantCategory>?> _fetchProductVariants(
    ...
    try {
      final List<Map<String, dynamic>>? variantList = await _productRepository
          .getProductVariants(
            token: widget.token,
      ...
      if (variantList != null) {
        List<ProductVariantCategory> variants = variantList
...
class _PaymentPageState extends State<PaymentPage> {
  ...
  Future<void> _fetchSubMethods(String categoryLabel) async {
    ...
    try {
      final url = Uri.parse(ApiEndpoints.getCoaMethodLink);
  ...
  Future<void> _fetchCalculation() async {
    ...
    try {
      final url = Uri.parse(ApiEndpoints.calculateTransaksiUrl);
      AppLogger.d('Transaction', 'CALCULATING API DEBUG 3');
      ...
      AppLogger.d(
        'Transaction',
        'STATUS ${response.statusCode} CALCULATING API RESPONSE: ${response.body}',
      );

      AppLogger.e('ERROR API', message);
