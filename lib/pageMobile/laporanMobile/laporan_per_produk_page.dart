...
class PendapatanPerProdukPage extends StatefulWidget {
  final String token;
  final String merchantId;

   PendapatanPerProdukPage({
    Key? key,
    required this.token,
  ...
  @override
  State<PendapatanPerProdukPage> createState() => _PendapatanPerProdukPageState();
}

class _PendapatanPerProdukPageState extends State<PendapatanPerProdukPage> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      final data = await getLaporanPerProduk(
        context,
        widget.token,
        keyword,
        _selectedSortValue,
  ...
  void _exportReport(String type) async {
    ...
    final result = await getLaporanPerProduk(
      context,
      widget.token,
      keyword,
      _selectedSortValue,
  ...
  Widget _buildReportList() {
    ...
    if (details.isEmpty) {
      return Center(
        child: Text("Tidak ada data laporan", style: heading3(FontWeight.w400, bnw500, 'Outfit')),
      );
    }
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return _buildProductCard(detail);
      },
    );
  }
          ...
          Row(
            children: [
              Container(
                ...
                child:  Icon(PhosphorIcons.package, color: bnw600, size: 20),
              ),
               SizedBox(width: 12),
              Expanded(
                child: Column(
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Text("Total", style: heading3(FontWeight.w600, Colors.white, 'Outfit')),
            Row(
              children: [
                Text(_formatCurrency(total), style: heading3(FontWeight.w600, Colors.white, 'Outfit')),
                 SizedBox(width: 8),
