...
class _LaporanStokPageState extends State<LaporanStokPage> {
  ...
  String _selectedSortLabel = "Stock Akhir A-Z";
  ...
  Future<void> _fetchData() async {
    ...
    try {
      final data = await getLaporanStok(
        context,
        widget.token,
        _selectedMerchants,
        keyword,
  ...
  Widget _buildReportList() {
    ...
    if (details.isEmpty) return Center(child: Text("Tidak ada data stok", style: heading3(FontWeight.w400, bnw500, 'Outfit')));
    return ListView.builder(
      padding:  EdgeInsets.all(16),
      itemCount: details.length,
      itemBuilder: (context, index) {
        final merchantData = details[index];
        final List stocks = merchantData['inventory_stock'] ?? [];
        return Column(
          children: stocks.map((stock) => _buildStockCard(merchantData['merchant_name'], stock, stocks.length)).toList(),
        );
      },
    );
  }

  Widget _buildStockCard(String? merchantName, Map<String, dynamic> stock, int totalBahan) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
           ...
           SizedBox(height: 16),
          Text(
            stock['inventory_name'] ?? 'Nama Inventaris',
          ...
          Row(
            children: [
              Expanded(
                child: _buildStockSubDetail("Stock Awal", "${stock['starting_stock'] ?? 0}"),
              ),
               SizedBox(width: 16),
              Expanded(
                child: _buildStockSubDetail("Stock Akhir", "${stock['final_stock'] ?? 0}"),
              ),
            ],
          ),
           SizedBox(height: 8),
          Row(
            children: [
              Expanded(
                child: _buildStockSubDetail(
                  "Stock Masuk",
                  "+${stock['total_entry_good'] ?? 0}",
  ...
  Widget _buildStockSubDetail(String label, String value, {Color? valueColor}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
